...
export default function Reports() {
  ...
  useEffect(() => {
    ...
    } catch { console.log('Error parsing user') }
  }, [])
    ...
    <Layout>
      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-400 text-sm mt-0.5">View and export your monthly financial reports</p>
        </div>

        {/* Month Selector */}
        <div className="flex items-center justify-between mb-6 bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-sm">
          <button onClick={prevMonth}
            className="w-10 h-10 rounded-xl border border-gray-200 dark:border-gray-700 flex items-center justify-center hover:bg-violet-50 hover:text-violet-600 transition text-lg font-bold text-gray-600 dark:text-gray-300">
            &lsaquo;
          </button>
          ...
          <button onClick={nextMonth} disabled={isCurrentMonth}
            className={`w-10 h-10 rounded-xl border border-gray-200 dark:border-gray-700 flex items-center justify-center transition text-lg font-bold ${isCurrentMonth ? 'text-gray-300 cursor-not-allowed' : 'text-gray-600 dark:text-gray-300 hover:bg-violet-50 hover:text-violet-600'}`}>
            &rsaquo;
          </button>
        </div>

        {/* Summary Cards — overflow safe, tappable */}
        <div className="grid grid-cols-3 gap-3 mb-6">
          {[
            ...
            { label: 'Balance', value: (balance >= 0 ? '+' : '') + currencySymbol + balance.toFixed(2), color: balance >= 0 ? 'text-violet-600' : 'text-red-500' },
          ].map((s, i) => (
            <button key={i} onClick={() => setModalData({ label: s.label + ' — ' + monthName, value: s.value })}
              ...
              <p className={`text-lg font-bold tabular-nums truncate ${s.color}`} title={s.value}>{s.value}</p>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-sm mb-6">
          ...
          <div className="flex gap-3">
            <button onClick={exportPDF}
              className="flex-1 bg-violet-600 text-white py-3 rounded-xl font-semibold hover:bg-violet-700 transition flex items-center justify-center gap-2 text-sm">
              PDF
            </button>
            <button onClick={exportCSV}
              className="flex-1 bg-green-600 text-white py-3 rounded-xl font-semibold hover:bg-green-700 transition flex items-center justify-center gap-2 text-sm">
              CSV
            </button>
          </div>
        </div>
        ...
        {categoryData.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-sm">
              <h3 className="text-sm font-semibold text-gray-800 dark:text-white mb-4">Category Breakdown</h3>
              <ResponsiveContainer width="100%" height={220}>
              ...
              <table className="w-full text-sm">
                  ...
                  {monthExpenses.sort((a, b) => new Date(b.date) - new Date(a.date)).map(e => (
                    <tr key={e.id} className="border-b border-gray-50 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-700/30">
                      ...
                      <td className="py-3 px-2">
                        <span className="bg-violet-100 dark:bg-violet-900 text-violet-600 dark:text-violet-300 px-2 py-0.5 rounded-full text-xs whitespace-nowrap">
                          {e.category}
                        </span>
                      </td>
                      <td className="py-3 px-2 text-gray-600 dark:text-gray-300 text-xs hidden md:table-cell">{e.description || '--'}</td>
                      <td className="py-3 px-2 text-right font-semibold text-gray-800 dark:text-white text-xs tabular-nums whitespace-nowrap">
                        {currencySymbol}{parseFloat(e.amount).toFixed(2)}
                      </td>
                    </tr>
                  ))}
                ...
                <tfoot>
                  <tr className="border-t-2 border-gray-200 dark:border-gray-700">
                    ...
                    <td className="py-3 px-2 text-right font-bold text-red-500 tabular-nums text-sm">{currencySymbol}{total.toFixed(2)}</td>
                  </tr>
                </tfoot>
              </table>
            </div>
