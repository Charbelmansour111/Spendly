  ...
  return (
    <div className="min-h-screen flex relative overflow-hidden" style={{ background: '#0d0518' }}>
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div style={{ position:'absolute',right:'0%',top:'0%',width:700,height:600,borderRadius:'50%',background:'radial-gradient(ellipse,rgba(124,58,237,0.45) 0%,transparent 68%)',filter:'blur(90px)',animation:'auth-orb-1 22s ease-in-out infinite',willChange:'transform' }} />
        <div style={{ position:'absolute',left:'-5%',bottom:'-10%',width:600,height:550,borderRadius:'50%',background:'radial-gradient(ellipse,rgba(99,102,241,0.3) 0%,transparent 68%)',filter:'blur(90px)',animation:'auth-orb-2 27s ease-in-out infinite',willChange:'transform' }} />
        <div style={{ position:'absolute',right:'20%',bottom:'10%',width:500,height:420,borderRadius:'50%',background:'radial-gradient(ellipse,rgba(167,139,250,0.2) 0%,transparent 68%)',filter:'blur(90px)',animation:'auth-orb-3 19s ease-in-out infinite',willChange:'transform' }} />
      </div>
      ...
      <div className="relative hidden md:flex flex-col justify-between w-5/12 p-12 text-white overflow-hidden">
        ...
        <div className="relative z-10 space-y-8" style={{ animation: 'slide-up 0.7s ease 0.1s both' }}>
          <div>
            <p className="text-violet-300 text-xs font-semibold uppercase tracking-widest mb-3">Everything you get, free</p>
            <h2 className="text-4xl font-bold leading-tight">Start your<br/>financial journey<br/>today.</h2>
          </div>
          <div className="space-y-5">
            {[
              { icon: '🧾', title: 'Expense & Income Tracking', desc: 'Log everything in seconds and see your true balance.' },
              { icon: '📸', title: 'Receipt Scanner', desc: 'Photograph any receipt — the amount logs itself via AI.' },
              { icon: '🤖', title: 'AI Finance Assistant', desc: 'Get personalized insights based on your real data.' },
              { icon: '📱', title: 'Works on Any Device', desc: 'Phone, tablet, or desktop — always in sync.' },
            ].map((f, i) => (
        ...
        <div className="w-full max-w-100" style={{ animation: 'slide-up 0.5s ease both' }}>
          ...
          <div className="bg-white/6 backdrop-blur-2xl border border-white/10 rounded-3xl p-8 shadow-2xl" style={{ boxShadow: '0 32px 80px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.08)' }}>
            <div className="mb-7">
              ...
              <p className="text-white/45 mt-1.5 text-sm">Free forever — no credit card required</p>
            </div>
            ...
            <div className="mt-6 pt-5 border-t border-white/8 text-center">
              <p className="text-white/35 text-sm">
                ...
                <a href="/login" className="text-violet-300 font-semibold hover:text-violet-200 transition">Sign in</a>
              </p>
            </div>
          </div>

          <p className="text-center text-white/20 text-xs mt-6">By signing up you agree to our <a href="/terms" className="underline hover:text-white/40">Terms</a> & <a href="/privacy" className="underline hover:text-white/40">Privacy</a></p>
        </div>
      </div>
    </div>
  )
