      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-sm text-center">
            ...
            <p className={`text-lg font-bold ${(data?.balance || 0) >= 0 ? 'text-violet-600' : 'text-red-500'}`}>
          ...
          {data?.achievements?.length > 0 ? (
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {data.achievements.map((a, i) => (
                <div key={i} className="flex items-center gap-3 bg-violet-50 dark:bg-violet-900/20 rounded-xl p-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-6">
            ...
            <div className="flex justify-between mb-4">
              {moods.map((m) => (
                <button key={m.value} onClick={() => saveMood(m.value)}
                  className={`flex flex-col items-center gap-1 p-2 rounded-xl transition ${selectedMood === m.value || data?.mood?.mood === m.value ? 'bg-violet-100 dark:bg-violet-900/30 scale-110' : 'hover:bg-gray-50 dark:hover:bg-gray-700'}`}>
            ...
            {moodResponseLoading && (
              <div className="animate-pulse h-8 bg-violet-50 dark:bg-violet-900/20 rounded-xl" />
            )}
          ...
          {/* AI Joke */}
          <div className="bg-linear-to-br from-yellow-50 to-orange-50 dark:from-gray-800 dark:to-gray-700 rounded-2xl shadow-sm p-6">
        ...
        {/* AI Quote */}
        <div className="bg-gradient-to-r from-violet-50 to-purple-50 dark:from-violet-900/20 dark:to-purple-900/20 rounded-2xl p-6 mb-6 border border-violet-100 dark:border-violet-800">
          <div className="flex justify-between items-center mb-3">
            <h3 className="text-sm font-semibold text-violet-600">💬 Motivational Quote</h3>
            <button onClick={fetchQuote} disabled={quoteLoading}
              className="text-xs text-violet-500 hover:text-violet-600 font-semibold disabled:opacity-50 flex items-center gap-1 bg-violet-100 dark:bg-violet-900/30 px-3 py-1.5 rounded-full transition">
              {quoteLoading ? '⏳' : '🔄'} New quote
            </button>
          </div>
          {quoteLoading ? (
            <div className="animate-pulse space-y-2">
              <div className="h-5 bg-violet-100 dark:bg-gray-600 rounded-xl w-3/4" />
              <div className="h-4 bg-violet-100 dark:bg-gray-600 rounded-xl w-1/3" />
            </div>
          ) : (
            <>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-6">
            ...
            <textarea value={monthlyGoal} onChange={e => setMonthlyGoal(e.target.value)}
              ...
              rows={3}
              className="w-full px-4 py-3 border border-gray-200 dark:border-gray-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-violet-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm resize-none" />
            <button onClick={saveGoal}
              className="mt-3 w-full bg-violet-600 text-white py-2 rounded-xl text-sm font-semibold hover:bg-violet-700 transition">
          ...
          <div className="flex flex-col md:flex-row gap-4 items-start">
            <div className="flex-1">
              <input type="text" placeholder="What are you saving for? (e.g. Dream vacation)"
                ...
                className="w-full px-4 py-3 border border-gray-200 dark:border-gray-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-violet-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm mb-3" />
              <label className="w-full flex items-center justify-center gap-2 border-2 border-dashed border-violet-300 dark:border-violet-700 rounded-xl py-6 cursor-pointer hover:bg-violet-50 dark:hover:bg-violet-900/20 transition">
                <span className="text-2xl">📸</span>
                <span className="text-sm text-violet-600 font-medium">Upload Image</span>
            ...
            {visionPreview && (
              <div className="md:w-48 w-full">
                ...
                {visionTitle && <p className="text-center text-sm font-medium text-violet-600 mt-2">{visionTitle}</p>}
              </div>
            )}
          </div>
        </div>
        ...
        <footer className="text-center py-6 text-gray-400 text-sm">
          <p>2026 <span className="text-violet-600 font-semibold">Spendly</span> — Track smarter, spend better</p>
        </footer>
