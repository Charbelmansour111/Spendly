            ...
            <button key={t.key} onClick={() => setTab(t.key)}
              ...
              {t.count > 0 && (
                <span className={`flex-shrink-0 text-xs px-1.5 py-0.5 rounded-full font-bold ${
                  tab === t.key ? 'bg-violet-100 text-violet-600' : 'bg-gray-200 dark:bg-gray-600 text-gray-500'
                }`}>{t.count}</span>
              )}
            </button>
          ))}
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-4">
              <div className="flex flex-wrap gap-2 items-center">
                ...
                <select value={filterCategory} onChange={e => setFilterCategory(e.target.value)}
                  className="px-3 py-2 border border-gray-200 dark:border-gray-600 rounded-xl text-sm bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-violet-400">
                  ...
                  {['All', 'Over', 'Close', 'OnTrack'].map(s => (
                    <button key={s} onClick={() => setFilterStatus(s)}
                      className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition ${
                        filterStatus === s
                          ? s === 'Over' ? 'bg-red-100 text-red-600' : s === 'Close' ? 'bg-orange-100 text-orange-600' : s === 'OnTrack' ? 'bg-green-100 text-green-600' : 'bg-violet-600 text-white'
                  ...
                  {['All', 'daily', 'weekly', 'monthly'].map(p => (
                    <button key={p} onClick={() => setFilterCategory(p)}
                      className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition capitalize ${
                        filterCategory === p ? 'bg-violet-600 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-500 hover:text-gray-700'
                      }`}>
                      {p}
                    </button>
                  ))}
                ...
                <div className="flex gap-1">
                  {[{ key: 'All', label: 'All' }, { key: 'Active', label: '✅ Active' }, { key: 'Paused', label: '⏸ Paused' }, { key: 'Triggered', label: '🔔 Triggered' }].map(s => (
                    <button key={s.key} onClick={() => setFilterStatus(s.key)}
                      className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition ${
                        filterStatus === s.key ? 'bg-violet-600 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-500 hover:text-gray-700'
                      }`}>
                      {s.label}
                    </button>
                  ))}
                  ...
                  .map(alert => {
                    ...
                    return (
                      <div key={alert.id} className={`bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-5 border-l-4 transition ${
                        !alert.is_active ? 'border-gray-200 opacity-60' : isTriggered ? 'border-red-500' : 'border-violet-500'
                      }`}>
                        <div className="flex items-start gap-3 mb-4 min-w-0">
                        ...
                        <div className="w-full bg-gray-100 dark:bg-gray-700 rounded-full h-2.5 mb-2">
                          <div className={`h-2.5 rounded-full transition-all duration-500 ${
                            isTriggered ? 'bg-red-500' : pct >= 80 ? 'bg-orange-400' : 'bg-violet-500'
                          }`} style={{ width: `${pct}%` }} />
                        </div>
                        ...
                        <div className="flex items-baseline justify-between gap-2 min-w-0">
                          ...
                          <span className={`flex-shrink-0 text-xs font-semibold ${isTriggered ? 'text-red-500' : 'text-violet-600'}`}>
                            {pct.toFixed(0)}%
                          </span>
                        </div>
                      </div>
